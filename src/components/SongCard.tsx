    ...
    <div
      ...
      className={`group relative p-3.5 rounded-2xl border transition-all duration-300 hover:-translate-y-1 cursor-pointer flex flex-col justify-between ${
        isCurrent
          ...
          : 'bg-white border-slate-200/90 hover:border-slate-300 hover:bg-slate-50/80 shadow-sm hover:shadow-md'
      }`}
    >
      {/* Artwork Container */}
      <div className="relative aspect-square w-full rounded-xl bg-slate-100 overflow-hidden mb-3 shadow-xs">
        {song.artwork ? (
          <img
            src={song.artwork}
            alt={song.title}
      ...
      {/* Info & Actions */}
      <div>
        <h4 className={`font-bold text-sm truncate ${isCurrent ? 'text-amber-300' : 'text-slate-900'}`}>
          {song.title}
        </h4>
        <p className={`text-xs truncate mt-0.5 ${isCurrent ? 'text-slate-300' : 'text-slate-500'}`}>{song.artist}</p>

        {/* Action Row */}
        <div className={`flex items-center justify-between mt-3 pt-2 border-t text-xs ${isCurrent ? 'border-slate-800' : 'border-slate-100'}`}>
          <button
            onClick={(e) => {
              e.stopPropagation();
