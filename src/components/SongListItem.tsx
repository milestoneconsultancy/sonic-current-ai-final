    ...
    <div
      className={`group relative flex items-center justify-between p-3 rounded-2xl border transition-all duration-200 ${
        isCurrent
          ...
          : 'bg-white border-slate-200/90 hover:bg-slate-100/70 hover:border-slate-300 text-slate-800 shadow-xs'
      }`}
    >
      ...
      <div className="flex items-center gap-3.5 min-w-0 flex-1">
        ...
        <div
          onClick={() => onPlay(song)}
          className="relative w-12 h-12 rounded-xl bg-slate-200 overflow-hidden shrink-0 shadow-xs cursor-pointer group/art"
        >
          {song.artwork ? (
            <img
              src={song.artwork}
        ...
        <div className="min-w-0 flex-1 cursor-pointer" onClick={() => onPlay(song)}>
          <h4 className={`font-bold text-sm truncate leading-snug ${isCurrent ? 'text-amber-300' : 'text-slate-900'}`}>
            {song.title}
          </h4>
          <p className={`text-xs truncate mt-0.5 ${isCurrent ? 'text-slate-300' : 'text-slate-500'}`}>
      ...
      <div className="flex items-center gap-1.5 shrink-0 ml-3">
        {/* Duration */}
        <span className={`text-xs font-mono hidden sm:inline-block w-12 text-right ${isCurrent ? 'text-slate-400' : 'text-slate-400'}`}>
        ...
        <button
          ...
          className={`p-2 rounded-xl transition ${
            isCurrent ? 'text-slate-300 hover:text-white hover:bg-slate-800' : 'text-slate-400 hover:text-teal-700 hover:bg-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`p-2 rounded-xl transition flex items-center gap-1 ${
            isDownloaded
              ? 'text-emerald-600 bg-emerald-50 font-bold border border-emerald-200'
              : isDownloading
        ...
        {onRemove && (
          <button
            ...
            title="Remove item"
            className={`p-2 rounded-xl transition ${
              isCurrent ? 'text-slate-400 hover:text-red-300' : 'text-slate-400 hover:text-red-500 hover:bg-slate-200'
            }`}
          >
            ×
          </button>
